import Link from 'next/link'
import NewsletterForm from '@/app/components/ui/NewsletterForm'
import FooterWordmark from '@/app/components/ui/FooterWordmark'

const DARK = '#0B1220'
const ACCENT = '#2563EB'

const NAVEGAR = [
  { href: '/', label: 'Inicio' },
  { href: '/servicios', label: 'Servicios' },
  { href: '/empleo', label: 'Buscar Empleo' },
  { href: '/nosotros', label: 'Nosotros' },
  { href: '/contacto', label: 'Contacto' },
]

const SERVICIOS_FOOTER = [
  'Outsourcing Personal',
  'Reclutamiento y Selección',
  'Gestión de Nómina',
  'Estudios Socioeconómicos',
  'Consultoría Laboral',
  'Bolsa de Empleo',
]

const SOCIALES = [
  {
    label: 'Instagram',
    href: 'https://www.instagram.com/hakamord/',
    bg: 'linear-gradient(135deg, #F58529, #DD2A7B, #8134AF)',
    icon: (
      <svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
        <rect x="2" y="2" width="20" height="20" rx="5" ry="5" />
        <circle cx="12" cy="12" r="4" />
        <circle cx="17.5" cy="6.5" r="1" fill="currentColor" stroke="none" />
      </svg>
    ),
  },
  {
    label: 'WhatsApp',
    href: 'https://wa.me/18296790671',
    bg: '#25D366',
    icon: (
      <svg width="17" height="17" viewBox="0 0 24 24" fill="currentColor">
        <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z" />
      </svg>
    ),
  },
  {
    label: 'Email',
    href: 'mailto:gestionhumanahakamo@gmail.com',
    bg: ACCENT,
    icon: (
      <svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
        <path d="M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2z" />
        <polyline points="22,6 12,13 2,6" />
      </svg>
    ),
  },
]

function FooterLink({ href, children }: { href: string; children: React.ReactNode }) {
  return (
    <Link
      href={href}
      className="group flex items-center gap-1.5 text-sm text-white/60 transition-colors hover:text-white"
    >
      <span className="transition-transform duration-200 group-hover:translate-x-1">{children}</span>
    </Link>
  )
}

export default function Footer() {
  return (
    <>
      {/* Newsletter — no es parte del footer */}
      <section
        className="relative overflow-hidden"
        style={{ backgroundColor: DARK, color: 'white' }}
      >
        <div className="h-px w-full" style={{ background: `linear-gradient(to right, transparent, ${ACCENT}, transparent)` }} />
        <div className="relative border-b border-white/10">
          <div className="mx-auto flex max-w-6xl flex-col items-center justify-between gap-6 px-6 py-10 lg:flex-row">
            <div className="text-center lg:text-left">
              <h3
                className="text-xl font-bold"
                style={{ fontFamily: 'var(--font-space-grotesk, sans-serif)' }}
              >
                Novedades sobre empleo y RRHH
              </h3>
              <p className="mt-1 text-sm text-white/55">
                Recibe oportunidades y tips de gestión de talento en tu correo.
              </p>
            </div>
            <NewsletterForm color={ACCENT} />
          </div>
        </div>
      </section>

      {/* Footer real */}
      <footer
        data-lanyard-front
        className="relative z-[50] overflow-hidden"
        style={{ backgroundColor: DARK, color: 'white' }}
      >
        <FooterWordmark />

        <div className="relative mx-auto max-w-6xl px-6 py-16">
          <div className="grid grid-cols-1 gap-12 lg:grid-cols-[1.2fr_1fr_1fr_1.2fr]">

            {/* Col 1 — Marca */}
            <div>
              <Link href="/" className="mb-5 inline-block transition-opacity hover:opacity-80">
                {/* eslint-disable-next-line @next/next/no-img-element */}
                <img
                  src="/hakamo-logo-blanco.png"
                  alt="Hakamo"
                  style={{ height: '90px', width: 'auto' }}
                />
              </Link>
              <p className="mb-7 max-w-xs text-sm leading-relaxed text-white/55">
                Empresa dominicana especializada en outsourcing de personal, reclutamiento y gestión
                de talento humano. Cumplimiento legal garantizado.
              </p>
              <div className="flex gap-3">
                {SOCIALES.map((s) => (
                  <a
                    key={s.label}
                    href={s.href}
                    target="_blank"
                    rel="noopener noreferrer"
                    aria-label={s.label}
                    className="flex h-10 w-10 items-center justify-center rounded-full transition-all hover:-translate-y-1 hover:shadow-lg"
                    style={{ background: s.bg }}
                  >
                    {s.icon}
                  </a>
                ))}
              </div>
            </div>

            {/* Col 2 — Navegar */}
            <div>
              <h4 className="mb-5 text-xs font-semibold uppercase tracking-[0.2em]" style={{ color: ACCENT }}>
                Navegar
              </h4>
              <ul className="space-y-3">
                {NAVEGAR.map((link) => (
                  <li key={link.href}>
                    <FooterLink href={link.href}>{link.label}</FooterLink>
                  </li>
                ))}
              </ul>
            </div>

            {/* Col 3 — Servicios */}
            <div>
              <h4 className="mb-5 text-xs font-semibold uppercase tracking-[0.2em]" style={{ color: ACCENT }}>
                Servicios
              </h4>
              <ul className="space-y-3">
                {SERVICIOS_FOOTER.map((titulo) => (
                  <li key={titulo}>
                    <FooterLink href="/servicios">{titulo}</FooterLink>
                  </li>
                ))}
              </ul>
            </div>

            {/* Col 4 — Contacto */}
            <div>
              <h4 className="mb-5 text-xs font-semibold uppercase tracking-[0.2em]" style={{ color: ACCENT }}>
                Contacto
              </h4>
              <ul className="space-y-4">
                <li>
                  <a
                    href="mailto:gestionhumanahakamo@gmail.com"
                    className="break-all text-sm text-white/60 transition-colors hover:text-white"
                  >
                    gestionhumanahakamo@gmail.com
                  </a>
                </li>
                <li>
                  <a
                    href="https://wa.me/18296790671"
                    target="_blank"
                    rel="noopener noreferrer"
                    className="block text-sm text-white/60 transition-colors hover:text-white"
                  >
                    829-679-0671 (WhatsApp)
                  </a>
                  <a href="tel:+18296796842" className="block text-sm text-white/60 transition-colors hover:text-white">
                    829-679-6842
                  </a>
                </li>
                <li className="text-sm text-white/60">Montecristi, República Dominicana</li>
              </ul>
              <Link
                href="/contacto"
                className="mt-6 inline-flex items-center gap-2 rounded-lg px-5 py-2.5 text-sm font-semibold text-white transition-all hover:opacity-90 hover:shadow-lg"
                style={{ backgroundColor: ACCENT }}
              >
                Escríbenos →
              </Link>
            </div>

          </div>
        </div>

        <div className="relative border-t border-white/10">
          <div className="mx-auto flex max-w-6xl flex-col items-center justify-between gap-3 px-6 py-5 sm:flex-row">
            <p className="text-xs text-white/35">
              © {new Date().getFullYear()} Hakamo. Todos los derechos reservados.
            </p>
            <div className="flex items-center gap-4">
              <Link href="/contacto" className="text-xs text-white/25 transition-colors hover:text-white/50">
                Política de Privacidad
              </Link>
              <span className="text-white/15">·</span>
              <Link href="/contacto" className="text-xs text-white/25 transition-colors hover:text-white/50">
                Términos de Servicio
              </Link>
              <span className="text-white/15">·</span>
              <a
                href="#top"
                className="flex items-center gap-1 text-xs text-white/25 transition-colors hover:text-white/50"
              >
                Volver arriba ↑
              </a>
            </div>
          </div>
        </div>
      </footer>
    </>
  )
}
