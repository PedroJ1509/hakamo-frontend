...
interface MaskedHeadingProps {
  text?: string
  /** Etiqueta con la que se renderiza, para llevar la semántica correcta. */
...
export default function MaskedHeading({
  ...
  // Se acota a una etiqueta concreta: con el tipo genérico, TypeScript resuelve
  // las props de la unión de todos los elementos a `never`.
  const Tag = tag as 'h2'

  return (
    <Tag
      ref={rootRef}
      className={`relative m-0 w-full p-0 text-balance antialiased ${className}`.trim()}
      style={{
        textAlign: align,
        fontWeight: weight,
    ...
    >
      {/* Capa de medición: da la posición de cada palabra al clipPath */}
      <span ref={measureRef} className="text-transparent">
        {words.map((word, i) => (
          <span
            ...
            className="inline-block whitespace-pre after:content-['_'] last:after:content-none"
          >
            {word}
            <i
              ref={(el) => {
