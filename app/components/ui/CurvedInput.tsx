...
const bentLinePath = (g: Geom, u0: number, u1: number, v: number) => `M ${fmt(g, u0, v)} ${edgeSeg(g, u1, v, true)}`
...
export default function CurvedInput({
  ...
  if (geom && layout) {
    ...
    content = (
      ...
      >
        ...
        <path className={styles.ring} d={bandPath} fill="none" stroke={accentColor} strokeWidth={borderWidth + 6} />
        <path d={bandPath} fill={bgColor} stroke={strokeColor} strokeWidth={borderWidth} />

        <path id={layoutPathId} d={layoutPath} fill="none" />

        {showIcon && (
          <g transform={`translate(${round2(ix)} ${round2(iy)}) rotate(${round2(iconAngle)})`} aria-hidden>
            {icon || (
              <>
        ...
        <g clipPath={`url(#${clipId})`}>
          ...
          {!display && placeholder && (
            <text style={{ fontSize: `${fontSize}px`, fontWeight: 500 }} fill={phColor} xmlSpace="preserve" aria-hidden>
        ...
        {showButton && (
          ...
          >
            <path className={styles.buttonBg} d={buttonPath} fill={accentColor} />
  ...
  return (
    <form
      ref={rootRef}
      className={`${styles.curvedInput} ${focused ? styles.focused : ''} ${className}`.trim()}
