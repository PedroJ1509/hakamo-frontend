...
export default function TextLoop({
  ...
  useLayoutEffect(() => {
    ...
    const measure = () => {
      ...
      try {
        ...
        const pad = Math.max(ribbonWidth, fontSize) / 2 + 12
        // Keep the full horizontal range so the path's off-canvas ends (and their
        // rounded stroke caps) stay clipped out of view; only crop vertically.
        setViewBox(`0 ${box.y - pad} ${VIEW_W} ${box.height + pad * 2}`)
      } catch {
        // getBBox can throw on detached/unrendered nodes; keep previous viewBox
      }
    }
  ...
  }, [d, unit, fontSize, fontWeight, letterSpacing, ribbonWidth])
